//
//  QuakeDataManager.swift
//  QuakeData
//
//  Created by Adrian Bolinger on 11/21/17.
...
class QuakeDataManager: NSObject {
  ...
  /*
   DispatchGroup coordinates async threads. Runnning the networking task on its own async
   thread and core data on its own async thread can lead to errors with larger data sets.
   */
